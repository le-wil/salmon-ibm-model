...
# Load ROMS dataset
roms_data = xr.open_dataset('/Users/leahwilczynski/Desktop/OceanParcels/InteractiveTuna/IBM/ROMS_flow_fields/stcc_his_osu_subset_0001.nc')

# Extract surface-level velocity fields (assuming s_rho=0 is the surface)
u_surface = roms_data['u'].isel(s_rho=-1).transpose('ocean_time', 'eta_u', 'xi_u')  # U component at the surface
v_surface = roms_data['v'].isel(s_rho=-1).transpose('ocean_time', 'eta_v', 'xi_v')  # V component at the surface
...
if(__name__=='__main__'):
    ...
    lon_start = np.random.uniform(lon_u.min() + 0.1, lon_u.max() - 0.1, npart)  # Use ROMS longitude bounds
    lat_start = np.random.uniform(lat_u.min() + 0.1, lat_u.max() - 0.1, npart)  # Use ROMS latitude bounds
    ...
    prey_data = np.ones((len(time_seconds), 156, 153))  # Uniform prey field with correct shape
    # Or use np.random.rand(len(time_seconds), 156, 153) for a random prey field
    ...
    output_file = pset.ParticleFile(
        name="output/ROMSFADPrey_no%d_npart%d_nfad%d_T%.2f_F%.2f_P%.2f_I%.2f_p%.1f_Pa%.1f.zarr" % (
    ...
    pset.execute(pyfunc=kernels,
                 pyfunc_inter=ikernels,
                              # 20 minute time step
                 runtime=rt, dt=1.2e3, output_file=output_file, recovery={ErrorCode.ErrorOutOfBounds: delete_particle}, verbose_progress=False)
